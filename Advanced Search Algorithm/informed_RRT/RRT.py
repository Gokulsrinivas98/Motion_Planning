...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center = [(self.start.col+self.goal.col)/2,(self.start.row+self.goal.row)/2,]
            # Compute rotation matrix from elipse to world frame - C
             
            ################
            a = (self.goal.col - self.start.col,self.goal.row - self.start.row)/abs(c_min)
            M = np.outer(a,[1,0])
            u,m,v = np.linalg.svd(M)
            M1 = np.array([[1.,0.],[0.,np.linalg.det(u)*np.linalg.det(v)]])
            C = u @ M1 @ v
            ################
            # Compute diagonal matrix - L
            L = np.diag([c_best/2,np.sqrt(c_best**2 - c_min**2)/2])
            # Cast a sample from a unit ball - x_ball
            x_ball = np.random.normal(0,0.75,2)
    ...
    def draw_map(self):
        ...
        '''
        # Create empty maphow to read pseudocode
