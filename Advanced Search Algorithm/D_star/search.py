...
class DStar:
    ...
    def process_state(self):
        ...
        node_X = self.min_node()
        k_old = self.get_k_min()
        self.delete(node_X)

        neighbors = self.get_neighbors(node_X)

        if k_old < node_X.h:
            self.insert(node_X, node_X.h)
            for neighbor in neighbors:
                if neighbor.parent == node_X:
                    self.insert(neighbor, node_X.h + self.cost(node_X, neighbor))
                else:
                    self.insert(neighbor, neighbor.h)
            for neighbor in neighbors:
                if neighbor.h <= k_old and node_X.h > neighbor.h + self.cost(node_X, neighbor):
                    node_X.parent = neighbor
                    node_X.h = neighbor.h + self.cost(node_X, neighbor)

        elif k_old == node_X.h:  
            for neighbor in neighbors:
                if neighbor.tag == "NEW" or \
                (neighbor.parent == node_X and neighbor.h != node_X.h + self.cost(neighbor, node_X)) or\
                     (neighbor.parent != node_X and neighbor.h > node_X.h + self.cost(neighbor, node_X)):
                    neighbor.parent = node_X
                    if neighbor.is_obs:
                        self.insert(neighbor, math.inf)
                    else:        
                        self.insert(neighbor, node_X.h + self.cost(neighbor, node_X))
